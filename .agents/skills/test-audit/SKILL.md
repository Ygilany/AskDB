...
whole subsystem's test surface (every test file a package or app owns), as its
own series of PRs.
...
routing (`turbo.json` `test` env, the `integrationSuite()` gate in
`scripts/test-utils/integration.mjs`, `.github/workflows/ci.yml`), and relevant
history. Read root and scoped `AGENTS.md` files first. When the test claims
dependency-backed behavior (AI SDK, database drivers, gray-matter, pg,
mysql2, mssql, better-sqlite3), inspect the dependency source or types
directly.
...
- core (`packages/core`);
- database layer (`packages/introspect`, `packages/connectors`,
  `packages/postgres`, `packages/mysql`, `packages/sqlite`,
  `packages/sqlserver`, `packages/prisma`);
- AI, config, and authoring libraries (`packages/ai*`, `packages/config`,
  `packages/client`, `packages/rag`, `packages/enrich`);
- apps, examples, scripts, and tooling (`apps/`, `examples/`, `scripts/`);
...
Keep a test when it independently enforces a public API, extension point
(`AskDialect`, `Connector`, `AiProviderAdapter`, `Retriever`/`VectorStore`),
protocol, config, migration, storage, security, SQL-safety or tenant
guardrail, platform, default, prompt-byte, Schema v2 artifact, package,
...
1. Run the smallest owner and sibling tests:
   ...
   (build first with `pnpm build` — packages resolve each other through `dist`).
   Integration suites run only when their env vars are set; see the
   Integration Tests section of `CONTRIBUTING.md`.
2. For removed source greps or plan assertions, run the executable script or
   dry-run that owns the real contract (for packaging and export assertions:
   `pnpm smoke:install`).
...
6. After final audit edits, review the full diff with fresh eyes (e.g. the
   `code-review` skill) before opening the PR.

## Landing and continuation

Commit, push, open a PR, or land only when authorized. Follow `AGENTS.md`:
add a changeset when a publishable package's shipped code changes (test-only
edits need none unless the Changesets status check requires one), and run
`pnpm smoke:install` and `pnpm preflight` before opening or updating a PR.

...
| `registry` | `AiRegistry` | yes | AI provider registry from `createAiRegistry([...providers])`. Used to construct the language model from config. |
| `schema` | `SchemaSource` | no | Default schema source. Falls back to `config.host.schemaJson` → `config.host.schemaPath` → `ASKDB_SCHEMA_JSON` → `ASKDB_SCHEMA_PATH` env. |

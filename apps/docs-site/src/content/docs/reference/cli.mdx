...
| `--schemas <list>` | Comma-separated database schemas to include (e.g. `public,sales`). For MySQL/MariaDB these are databases; each becomes its own namespace, and the flag overrides `introspection.providerConfig.mysql.databases`. |

...
| Flag | Description |
| --- | --- |
| `--ai-provider <name>` | `openai` \| `anthropic` \| `google` \| `azure` \| `foundry` \| `gateway` |
...
| Flag | Description |
| --- | --- |
| `--studio-execute` | Enable Studio execute (query playground): writes a `studio.execute` block with `enabled: true`. Off by default. |
| `--no-studio-execute` | Disable Studio execute (default). |
...
If the generated SQL references identifiers your schema marks `sensitive`, `askdb ask` prints a `Warning:` line to stderr listing them and still prints the SQL. The check is [`validateSensitiveReferences`](/reference/core-api/#validatesensitivereferencessql-schema-options) from `@askdb/core`, run by `ask()` in warn mode; embedding hosts can switch it to `strict` or call it directly on cached SQL.
...
```bash
askdb-rag setup-store --pg-url "$DATABASE_URL" --embedder openai
```
...
| `--embedder <id>` / `--embedder-model <id>` | Used to pick the default dimensions, the same way `index` does. |
| `--dimensions <n>` | Embedding dimensions. Defaults to the embedder's: `64` for the default `mock` embedder, `1536` for `--embedder openai` (`text-embedding-3-small`), `3072` for `text-embedding-3-large`. |

If the table already exists with a different `vector(n)` dimension, the command fails with an error rather than keeping the old dimension silently.
...
| `--dimensions <n>` | Embedding dimensions. Also sets the `mock` embedder's vector size (default `64`). |
| `--pg-url` / `--pg-table` | pgvector connection when `--store pgvector`. The table is provisioned (idempotently) and its dimensions checked before indexing. |
| `--force` | Re-embed every chunk, even ones the store already holds. |

Only chunks the store doesn't already hold (same id and content hash) are embedded. `schema.lock.json` records the embedder id, store, and dimensions; if any of them changed, everything is re-embedded.
...
| `--store`, `--embedder`, `--embedder-model`, `--dimensions`, `--pg-url`, `--pg-table`, `--file-path` | Must match the `index` run. `query` fails if the embedder id or dimensions differ from `schema.lock.json`. `--store memory` isn't supported: a memory store only lives inside one process. |

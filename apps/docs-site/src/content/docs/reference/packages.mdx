...
Browser-based schema-authoring UI. The CLI's `askdb studio` command lazy-imports this package. Schema browsing, enrichment, and SQL generation do not require a database driver; the optional Postgres execute path requires `pg` in the project running Studio.

...
Optional AI provider helpers. Install this when you want AskDB to construct AI SDK language or
embedding models from `askdb.config.*` / env maps. Skip it when you already construct your own AI
SDK model and pass it to `ask()`.
...
| Symbol | Purpose |
| --- | --- |
| `createAskDbLanguageModelFromEnv` | Build a language model from `getAskDbRuntimeConfig().ai.aiEnv`. |
| `createAskDbEmbeddingModelFromEnv` | Build an embedding model from the same config with embedding-specific defaults. |
| `resolveAskDbAiConfig` / `resolveAskDbEmbeddingConfig` | Resolve provider config without constructing a model. |
| `askDbAiKeyMissingMessage` | Shared human-readable missing-key guidance. |
...
| `@askdb/ai` | AI SDK core package (`ai`) + optional provider packages |

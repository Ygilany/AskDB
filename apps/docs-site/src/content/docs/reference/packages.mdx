...
<InstallTabs pkgs="@askdb/client @askdb/ai @askdb/ai-openai @askdb/config" />

**Key exports**

| Symbol | Purpose |
| --- | --- |
| `createAskDb(options)` | Build a client bound to a config + AI registry. Returns `{ ask, reload }`. Options include `unknownDialect: "throw" \| "fallback-postgres"` for artifacts recorded by an unrecognized provider. |

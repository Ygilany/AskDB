...
<p>First-party Node entrypoints load <code>{".env"}</code> and optional <code>{"askdb.config.*"}</code> / <code>{".config/askdb.*"}</code> through <code>{"@askdb/config"}</code> before reading canonical environment variables. See <a href="../environment/">Environment</a> and <a href="https://github.com/Ygilany/AskDB/blob/main/docs/adrs/0005-askdb-config-and-env-bootstrap.md">ADR 0005</a>.</p>
...
<div className="package-table">
  <a href="../environment/"><strong>@askdb/config</strong><span>Optional env bootstrap: <code>{"askdb.config.*"}</code> discovery and mapping into <code>{"process.env"}</code>.</span></a>

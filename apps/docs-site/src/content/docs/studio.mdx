...
Execute is a development tool: it runs whatever SQL the Playground sends, using the connection you configure. Postgres and MySQL queries run in a read-only transaction, and SQLite files are opened read-only. SQL Server queries get no read-only wrapper. For every engine, point `studio.execute` at a read-only, least-privilege login and a non-production database.

...
Every option `ask()` takes — modes, tenant scope, retrieval, sensitive-field handling — is in the [ask() options reference](/reference/ask-options/). The same options work as per-call overrides on `askdb.ask(question, overrides)`.

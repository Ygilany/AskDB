...
Set `ai.reasoning` in `askdb.config.ts` (see the [config reference](/reference/config/#ai-reasoning--reasoninglatency-effort)) for the config-driven path. Programmatically, resolve the portable effort to provider `providerOptions` with `@askdb/ai`'s registry, then forward it to `ask()`'s `deps.providerOptions` — `@askdb/core` forwards it verbatim to `generateText` and never interprets it:
...
Skip `ai.reasoning`/`resolveProviderOptions` entirely and current behavior is unchanged — no `providerOptions` are sent, and the provider/model uses its own default.

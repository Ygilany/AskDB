...
- The SQL checks: must start with `SELECT` or `WITH`, single statement, no comments, no write or DDL keywords, and the heuristic tenant and sensitive-column checks. A few engines block extra keywords. See [Safety boundaries](/concepts/safety-boundaries/).

...
// Prefer params (full positional array) when parameterized extras are present.
...
carries advisory metadata (role, department) into the prompt. It isn't enforced.

<Aside type="note" title="Not yet enforced: descendants and tenantFilters">
`subtree` doesn't expand descendants yet. The prompt asks the model to include
them, but only the `rootIds` you pass are bound into the SQL. Until expansion
ships, resolve the descendant IDs in your application and pass them with
`kind: "ids"`.

`TenantScope` also accepts a `tenantFilters` field. AskDB validates its shape
but doesn't use it for anything, so it has no effect on the prompt, the check,
or the returned SQL. Don't rely on it.
</Aside>
...
When `parameterize` is on (default), business markers are allocated first and tenant markers continue from `N+1` in `sql-params` mode. `tenantParams` stays tenant-only — execute with `params` when using the new fields.

...
`:tenant_…` token. Placeholders are case-sensitive: `:TENANT_ORGANIZATION_IDS`
isn't substituted, so it throws the same error. With several IDs, `= :p` becomes `IN (…)` and `!= :p`

...
function readManifest(bundlePath: string): BundleManifest {
  ...
  if (!existsSync(manifestPath)) {
    throw new Error(
      `@askdb/postgres: export bundle is missing manifest.json at ${manifestPath}`,
    );
  }
  ...
  if (!isObject(parsed)) {
    throw new Error(
      `@askdb/postgres: manifest.json must be a JSON object`,
    );
  }
  if (parsed.engine !== "postgres") {
    throw new Error(
      `@askdb/postgres: unsupported export bundle engine '${String(parsed.engine)}' (expected 'postgres')`,
    );
  }
  if (parsed.version !== POSTGRES_TEMPLATE_VERSION) {
    throw new Error(
      `@askdb/postgres: unsupported export bundle version '${String(parsed.version)}' (expected ${POSTGRES_TEMPLATE_VERSION})`,
    );
  }
...
function validateFilesMap(
  ...
  if (!isObject(value)) {
    throw new Error(
      `@askdb/postgres: ${manifestPath} field 'files' must be an object`,
    );
  }
  const out: Partial<Record<PostgresSqlTemplateName, string>> = {};
  const known = new Set(POSTGRES_TEMPLATES.map((tpl) => tpl.name));
  for (const [name, file] of Object.entries(value)) {
    if (!known.has(name as PostgresSqlTemplateName)) {
      throw new Error(
        `@askdb/postgres: manifest.json references unknown template '${name}'`,
      );
    }
    if (typeof file !== "string" || file.length === 0) {
      throw new Error(
        `@askdb/postgres: manifest.json file for '${name}' must be a non-empty string`,
      );
    }
    out[name as PostgresSqlTemplateName] = file;
  }
  return out;
}
...
function resolveTemplateFile(
  ...
  if (explicit) {
    ...
    if (!existsSync(path)) {
      throw new Error(
        `@askdb/postgres: export bundle is missing file '${explicit}' for template '${name}'`,
      );
    }
    return path;
  }
  ...
  throw new Error(
    `@askdb/postgres: export bundle is missing ${name}.csv or ${name}.json`,
  );
}
...
function readTemplateJson(path: string, tpl: SqlTemplate): Record<string, unknown>[] {
  ...
  if (!Array.isArray(parsed)) {
    throw new Error(
      `@askdb/postgres: ${path} must contain a JSON array of row objects`,
    );
  }
...
function readTemplateCsv(path: string, tpl: SqlTemplate): Record<string, unknown>[] {
  ...
  if (rows.length === 0) {
    throw new Error(
      `@askdb/postgres: ${path} must include a header row`,
    );
  }
  ...
  return data
    ...
    .map((row, rowIndex) => {
      ...
      tpl.columns.forEach((column, index) => {
        record[column] = coerceField(tpl.name as PostgresSqlTemplateName, column, row[index] ?? "");
      });
...
function normalizeRecord(
  ...
  if (!isObject(row)) {
    throw new Error(
      `@askdb/postgres: ${path} row ${index + 1} must be an object`,
    );
  }
  ...
  for (const column of tpl.columns) {
    if (!(column in row)) {
      throw new Error(
        `@askdb/postgres: ${path} row ${index + 1} is missing column '${column}'`,
      );
    }
    record[column] = coerceField(tpl.name as PostgresSqlTemplateName, column, row[column]);
  }
  return record;
}
...
function assertHeader(path: string, tpl: SqlTemplate, header: string[]): void {
  ...
  if (
    ...
    throw new Error(
      `@askdb/postgres: ${path} header mismatch for template '${tpl.name}' (expected [${expected.join(", ")}], got [${header.join(", ")}])`,
    );
  }
}

function coerceField(
  template: PostgresSqlTemplateName,
  column: string,
  value: unknown,
): unknown {
...
function isNumberField(template: PostgresSqlTemplateName, column: string): boolean {
  return (
...
function isBooleanField(template: PostgresSqlTemplateName, column: string): boolean {
  return (
...
function parseBoolean(
  ...
  throw new Error(
    `@askdb/postgres: cannot parse boolean '${value}' for ${template}.${column}`,
  );
}
...
function parseJsonFile(path: string): unknown {
  try {
    ...
    throw new Error(
      `@askdb/postgres: failed to parse JSON file ${path}: ${message}`,
    );
  }
}
...
function parseCsv(input: string): string[][] {
  ...
  if (inQuotes) {
    throw new Error("@askdb/postgres: unterminated quoted CSV field");
  }

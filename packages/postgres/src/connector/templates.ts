...
import type { SqlTemplateBundle } from "@askdb/introspect";

/**
 * Stable identifier for a Postgres catalog query template. Used by the air-gapped path
 * to map bundle files (CSV/JSON) to template results.
 */
export type PostgresSqlTemplateName =
  | "schemas"
  | "tables"
  | "columns"
  | "primary_keys"
...
/** Postgres-specific narrowing of the engine-agnostic `SqlTemplate` shape. */
export type PostgresSqlTemplate = {
...
const SCHEMAS_TEMPLATE: PostgresSqlTemplate = {
  name: "schemas",
  columns: ["schema_name"],
  sql: `
...
const TABLES_TEMPLATE: PostgresSqlTemplate = {
  name: "tables",
  columns: [
    "schema_name",
    "table_name",
...
const COLUMNS_TEMPLATE: PostgresSqlTemplate = {
  name: "columns",
  columns: [
    "schema_name",
    "table_name",
...
const PRIMARY_KEYS_TEMPLATE: PostgresSqlTemplate = {
  name: "primary_keys",
  columns: [
    "schema_name",
    "table_name",
...
const UNIQUE_CONSTRAINTS_TEMPLATE: PostgresSqlTemplate = {
...
const CHECK_CONSTRAINTS_TEMPLATE: PostgresSqlTemplate = {
...
const INDEXES_TEMPLATE: PostgresSqlTemplate = {
  name: "indexes",
  columns: [
    "schema_name",
    "table_name",
...
const ENUMS_TEMPLATE: PostgresSqlTemplate = {
  name: "enums",
...
const SEQUENCES_TEMPLATE: PostgresSqlTemplate = {
  name: "sequences",
  columns: [
    "schema_name",
    "sequence_name",
...
const VIEWS_TEMPLATE: PostgresSqlTemplate = {
  name: "views",
 ...
 */
const COMMENTS_TEMPLATE: PostgresSqlTemplate = {
  name: "comments",
  columns: [
    "object_type",
    "schema_name",
...
export const POSTGRES_TEMPLATES: ReadonlyArray<PostgresSqlTemplate> = [
  SCHEMAS_TEMPLATE,
  TABLES_TEMPLATE,
  COLUMNS_TEMPLATE,
...
export function findTemplate(name: PostgresSqlTemplateName): PostgresSqlTemplate {
  const t = POSTGRES_TEMPLATES.find((tpl) => tpl.name === name);
  if (!t) {
    throw new Error(`@askdb/postgres: unknown template '${name}'`);
  }
  return t;
}

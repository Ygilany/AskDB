...
export type { TabularResult } from "@askdb/core";
 ...
 * a custom `AskDbExecutor` must be able to import `@askdb/postgres` (for the dialect or
 * connector) without having `pg` installed. The cost of the missing peer is paid only when the
 * built-in executor is actually invoked.
 *

...
import type { IntrospectionFilters, IntrospectionInput } from "./types.js";
...
async function runWithOutput(
  ...
  if (opts.print) {
    const result = await introspect(input);
...
function buildInput(opts: CliOptions): IntrospectionInput {

...
- [`packages/ai`](packages/ai) (`@askdb/ai`) — AI provider registry with built-in OpenAI, Azure OpenAI / Foundry, Google Gemini, Anthropic, and Vercel AI Gateway providers (`@ai-sdk/*` packages are optional peers).
- [`packages/ai-openai`](packages/ai-openai), [`packages/ai-azure`](packages/ai-azure), [`packages/ai-google`](packages/ai-google), [`packages/ai-anthropic`](packages/ai-anthropic) — **deprecated** re-export shims of the matching `@askdb/ai` built-in provider; removed before 1.0.
...
- [`packages/connectors`](packages/connectors) (`@askdb/connectors`) — **deprecated** shim; the connector registry moved into `@askdb/introspect`.
- [`packages/introspect`](packages/introspect) (`@askdb/introspect`) — engine-agnostic introspection orchestrator, Schema v2 renderer, connector registry, and the `@askdb/introspect/kit` engine helpers.

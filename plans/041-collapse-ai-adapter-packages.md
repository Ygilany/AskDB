...
- **Plan 034 (CommonJS requireability) interacts here.** 034 adds a `default` condition to every export map and a guard test asserting no top-level await in published sources. This restructure must preserve both: keep `default` last in `packages/ai`'s export map, and keep the lazy provider loads inside functions — a top-level `await import(...)` would break `require('@askdb/ai')` and fail 034's guard. (034 emits no CommonJS build, so there is no second build output to verify.)

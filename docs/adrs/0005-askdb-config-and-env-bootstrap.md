...
6. **`@askdb/config`** is the **single package that reads `process.env`** directly. Env-aware AI functions in `@askdb/ai` (e.g. `resolveAskDbAiConfig`, `createAskDbLanguageModelFromEnv`) accept the env map as an **explicit, required parameter** (typically **`getAskDbRuntimeConfig().ai.aiEnv`**). Library packages obtain all runtime configuration through **`getAskDbRuntimeConfig()`** and must not call `env()`, `requiredEnv()`, or read `process.env` — `env()` is reserved for end-user `askdb.config.*` authoring only.

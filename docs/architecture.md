...
| CLI workflow | `askdb` | Pulls first-party integrations and surfaces used by the `askdb` binary, including core, ai, introspect, postgres, prisma, studio, tui, selected AI SDK providers, and `pg`. | Environment variables choose which runtime paths are active. | Batteries-included product surface, not the smallest library install. |
| Terminal enrichment | `@askdb/tui` | Pulls `@askdb/core`, `@askdb/ai`, `@askdb/enrich`, Ink, React, and selected AI SDK providers. | `OPENAI_API_KEY` enables suggestions; manual authoring works without live suggestions. | Operates on Schema v2 files only. |
| Local browser Studio | `@askdb/studio` | Pulls core, ai, enrich, postgres, rag, React UI dependencies, and selected AI SDK providers. | `OPENAI_API_KEY` enables suggestions/sample generation; RAG provider choices are runtime config. | Local authoring UI, sample SQL checks, and local RAG exploration. |

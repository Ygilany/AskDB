...
| --- | --- | --- | --- | --- |
| Minimal Postgres SQL generation | `@askdb/core`, `@askdb/postgres` | `@askdb/postgres` pulls `@askdb/core`, `@askdb/introspect`, and `ai` as package dependencies. | Add a model provider such as `ai` and `@ai-sdk/openai` when your runtime creates the model. `pg` is not needed. | Uses `ask()` plus `postgresDialect`; SQL execution remains host-owned. |

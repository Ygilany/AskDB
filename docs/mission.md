...
- **Installable contract** — `@askdb/core` is published to npm with a documented, semver-stable surface. Wrappers (CLI, TUI, RAG, HTTP, MCP, future SDK) depend on it as consumers, not co-located private code. See [`docs/integration/installable-package.md`](integration/installable-package.md) for BYO model + BYO executor recipes.

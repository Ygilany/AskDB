...
- **Schema loader** — `loadSchema(path)` autodetects between a schema artifact directory, bundled JSON, or a `schema.json` path inside a directory; returns a unified `NormalizedSchema`
...
// NormalizedSchema — the runtime representation of a loaded schema
interface NormalizedSchema {
  schemaId: string              // stable identifier for the schema artifact
...
interface NormalizedTable {
  id: string                    // e.g. "table:public.orders"
  name: string
  schema: string                // DB schema name, e.g. "public"
  description?: string
  aliases?: string[]
  primaryEntity?: string
  columns: NormalizedColumn[]
  relationships?: NormalizedRelationship[]
  commonQueryLanguage?: string  // verbatim "Common query language" H2 body
...
interface NormalizedColumn {
  id: string                    // e.g. "table:public.orders#total"
  name: string
  type: string
  description?: string
  aliases?: string[]
...
- Loader accepts a schema artifact directory, bundled JSON, and a path to `schema.json` inside a directory; returns a normalized shape in each case.
...
- Golden DDL output for the describable schema fixture includes table descriptions, aliases, `Common query language` blocks; sensitive describable-layer fields are absent.

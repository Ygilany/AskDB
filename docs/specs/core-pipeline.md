...
- SQL validation and dialect-aware guardrails — heuristic text checks, not a parser: must start with `SELECT`/`WITH`, single statement, no comments, no write/DDL keywords (base denylist plus per-dialect extras). Defense in depth, not a security boundary
